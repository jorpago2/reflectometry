...
test("ships one English-only material-agnostic multilayer interface", async () => {
  ...
  assert.match(files[1], /from "@carbon\/react"/);
  assert.match(files[1], /from "@carbon\/react\/icons"/);
  assert.match(files[1], /<TabsVertical>/);
  assert.match(files[1], /<TabListVertical className="configuration-rail"/);
  for (const tab of ["Measurement", "Layer stack", "Fit"]) assert.match(files[1], new RegExp(`>${tab}<\/Tab>`));
  assert.equal([...files[1].matchAll(/<TabPanel className="configuration-panel">/g)].length, 3);
  assert.doesNotMatch(files[1], /WorkflowSection|DisclosurePanel/);
  assert.match(files[1], /<ToggletipContent/);
